    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        function updateUI() {
            ...
            document.querySelectorAll(".popup-questions__questions-answer.selected").forEach((answer => {
                const price = parseInt(answer.dataset.price);
                total += isNaN(price) ? 0 : price;
            }));
            priceElem.textContent = total;
            let answeredCount = 0;
            questionBlocks.forEach((block => {
                const selected = block.querySelectorAll(".popup-questions__questions-answer.selected");
                if (selected.length > 0) answeredCount++;
            }));
            const progress = answeredCount / questionBlocks.length * 100;
            progressBarFill.style.width = progress + "%";
            const allAnswered = answeredCount === questionBlocks.length;
            contactBtn.classList.toggle("active", allAnswered);
            contactBtn.disabled = !allAnswered;
        }
        document.querySelectorAll(".popup-questions__questions-answer").forEach((answer => {
